...
void CGameContext::CreateDamageInd(vec2 Pos, float Angle, int Amount, int CltMask)
{
	...
	for(int i = 0; i < Amount; i++)
	{
		...
		NETEVENT_DAMAGEIND *pEvent = (NETEVENT_DAMAGEIND *)m_Events.Create(NETEVENTTYPE_DAMAGEIND, sizeof(NETEVENT_DAMAGEIND), CltMask);
		if(pEvent)
		{
...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage)
{
	...
	if (!NoDamage)
	{
		...
		//	if((int)Dmg)
			...
			if (apEnts[i]->GetFreezeTicks() <= 0 && (int)Dmg){
				if(apEnts[i] != GetPlayerChar(Owner))
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	int SELECTED_EMOTE = EMOTE_NORMAL;
	m_VoteUpdate = true;
}
	...
	if(MsgID == NETMSGTYPE_CL_SAY)
	{
		...
		if (pMsg->m_pMessage[0] == '/')
		{
			if (str_comp_num(pMsg->m_pMessage, "/info", 5) == 0)
				SendChatTarget(ClientID, "This is OpenFNG, written by Rem1x");
			else if (str_comp(pMsg->m_pMessage, "/cmdlist") == 0)
				SendChatTarget(ClientID, "NO Commands");
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	else if(str_comp(g_Config.m_SvGametype, "FM") == 0)
